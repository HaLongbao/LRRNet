...
from utils.loss import SoftLoULoss, SoftIoUL1NromLoss
...
def parse_args():
    ...
    # Dataset parameters
    #
    parser.add_argument('--base-size', type=int, default=512, help='base size of images')
    parser.add_argument('--crop-size', type=int, default=512, help='crop size of images')
    ...
    # logger
    args.logger = setup_logger("Deep Patch-Free Low-Rank Representation Network", args.save_folder, 0, filename='log.txt')
    return args
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.dataset == 'sirstaug':
            ...
            trainset = IRSTD1kDataset(base_dir=r'/home/avp/hcaUnetV2/datasets/IRSTD-1k', mode='train', base_size=args.base_size)
            valset = IRSTD1kDataset(base_dir=r'/home/avp/hcaUnetV2/datasets/IRSTD-1k', mode='test', base_size=args.base_size)
        ...
        self.softioul1 = SoftIoUL1NromLoss()
    ...
    def training(self):
        ...
        for epoch in range(args.epochs):
            for i, (data, labels) in enumerate(self.train_data_loader):
                ...
                loss_softioul1 = self.softioul1(out_T, labels)
                loss_mse = self.mse(out_D, data)
                gamma = torch.Tensor([0.1]).to(self.device)
                loss_all = loss_softiou  + torch.mul(gamma, loss_mse) + loss_softioul1
